...
export const Chat = () => {
  const { data: chatList, loading, error, updateQuery } = useUsersQuery();
  const [selectedChat, setSelectedChat] = useState<number | undefined>(
    undefined
  );

  const handleNewMessage = useCallback(
    (newMessageObj: ChatMessageType) => {
      if (!selectedChat || !chatList) return;
      const newUserChat: ChatListDataType = {
        id: chatList[selectedChat].id,
        name: chatList[selectedChat].name,
        lastMessage:
          (newMessageObj.sender ? "" : "You: ") + newMessageObj.message,
  ...
  return (
    <div className="chat">
      ...
      <div className="chat-right">
        {selectedChat ? (
          <ChatRightFold
            chatData={chatList?.[selectedChat]}
